...
class AudioStreamManager:
    ...
    def start(self):
        ...
        event_bus.emit(
            EventTypes.AUDIO_CAPTURE_START,
            {
    ...
    def stop(self):
        ...
        if self.audio:
            try:
                # Give PyAudio a moment to clean up properly
                import time
        ...
        while self.running:
            try:
                ...
                audio_data = self.stream.read(
                    AUDIO_CONFIG["chunk_size"], exception_on_overflow=False
                )
                chunks_captured += 1
                ...
                for consumer in consumers_copy:
                    ...
                    try:
                        ...
                        if self.running:
                            ...
                            print(
                                f"{colors['error']}Consumer error: {e}{colors['reset']}"
                            )
                            ...
                            with self.consumers_lock:
                                if consumer not in self.failed_consumers:
                                    ...
                                    logger.warning(
                                        "Consumer marked as failed and will be skipped"
                                    )
    ...
    def pause_microphone(self):
        ...
        with self.consumers_lock:
            ...
            colors = DISPLAY_CONFIG["colors"]
            logger.info(
                f"Microphone paused - stored {len(self.paused_consumers)} consumers"
            )

            # Emit microphone pause event
            event_bus.emit(
                EventTypes.AUDIO_MICROPHONE_PAUSE,
    ...
    def resume_microphone(self):
        ...
        with self.consumers_lock:
            ...
            colors = DISPLAY_CONFIG["colors"]
            logger.info(
                f"Microphone resumed - restored {len(self.consumers)} consumers"
            )

            # Emit microphone resume event
            event_bus.emit(
                EventTypes.AUDIO_MICROPHONE_RESUME,
    ...
    def broadcast_microphone_state(self):
        ...
        with self.consumers_lock:
            event_bus.emit(
                EventTypes.AUDIO_MICROPHONE_STATE,
                {
